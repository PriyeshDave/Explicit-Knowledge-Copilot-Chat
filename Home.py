...
import json
import clg_gpt3 as open_ai_gpt3
import re
import os
...
with col_main_2:
    ...
    st.markdown(
                    ...
                    <div style='text-align: center; font-size: 20px;'>
                        This intuitive feature enhances your data querying experience by leveraging comprehensive PDF documents that contain detailed information about various departments and key terms.
                        Whether you need insights into asset utilization scores or other critical metrics, the Chat Doc App provides you with the context and information you need to formulate precise and effective queries. 
                        Dive into your data with confidence and clarity, knowing that the information you seek is always at your fingertips."
                    </div>
                    """, 
                    unsafe_allow_html=True
        )   
...
def get_openai_response(question,texts=doc_to_vect,faiss_index=faiss_index):
    llm=OpenAI(openai_api_key=os.getenv("OPENAI_API_KEY"),temperature=0.5)
    docs = faiss_index.similarity_search(question, get_top_k=1)
    source_docs = list(set(i.metadata['source_file_name'] for i in docs))[0]
    source_document_name = source_docs.split('.')[0] + '.pdf'

    source_documents="\n \n \nData Citation Details: \nSource Document Section: " + source_document_name
    #source_documents="\n \n \nData Citation Details: \nSource Documents:\n "+',\n'.join(list(set([i.metadata['source_file_name'] for i in docs])))

    chain = load_qa_chain(llm,chain_type="stuff")
    question = "User Text : " + question + """ \n You are a document insights generating bot.
    Analyise the context document and generate human like response to the use question based on the context. 
    If user is asking for any task give detailed steps.
    Do not guess or add any information outside the contex.
    """
    ...
    return response + source_documents, source_document_name

def get_pdf_download_link(file_path, file_name):
                with open(file_path, "rb") as f:
                    pdf_data = f.read()
                
                # Convert the PDF data to a downloadable format
                st.download_button(
                ...
                if new_question not in st.session_state[index_questions]:
                    ...
                    for key in st.session_state[index_questions]:
                        if new_question == key:
                            ...
                            output, source_document_name = get_openai_response(new_question)
        ...
        with chat_col:
            if st.session_state["all_result"]:
                ...
                if source_document_name != '':
                    pdf_file_path = f'./data-bank/{source_document_name}'
                    if os.path.exists(pdf_file_path):
                        get_pdf_download_link(pdf_file_path, source_document_name)
                    else:
...
with col_main_2:
    ...
    st.markdown("<h3 style='text-align: left;'> 💡 Smart Query Suggestions </h3>", unsafe_allow_html=True)
